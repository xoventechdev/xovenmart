...
import { Shield, Calendar, ChevronRight, Filter } from "lucide-react";
...
export default function AdminActionsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin", "audit", "admin-actions", page, filters],
    queryFn: () =>
      api.get(`/admin/audit/admin-actions?${query.toString()}`) as Promise<AuditPage>,
  });
      ...
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink-900 dark:text-ink-900">
            {t("অ্যাডমিন অ্যাকশন", "Admin Actions")}
          </h1>
          <p className="mt-1 text-sm text-ink-500">
            {t("শুধুমাত্র ADMIN রোলের অ্যাকশন", "Actions performed by ADMIN role only")}
          </p>
        </div>
        {data && (
          <Badge variant="danger" className="text-xs">
            <Shield className="mr-1 h-3 w-3" /> {data.total} {t("অ্যাকশন", "actions")}
          </Badge>
        )}
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-3 md:grid-cols-4">
            <Input
              placeholder={t("এনটিটি (order, product...)", "Entity (order, product...)")}
            ...
            />
            <Input
              placeholder={t("অ্যাকশন (create, update...)", "Action (create, update...)")}
        ...
        <CardContent className="p-2 sm:p-3">
          {isLoading ? (
            ...
            <p className="py-8 text-center text-sm text-ink-500">
              {t("কোন অ্যাকশন নেই", "No actions found")}
            </p>
          )}
        </CardContent>
      </Card>
